...
master_version := $(shell git show master:lisp/spookfox.el | grep 'spookfox-version' | grep -o '"[0-9\.]*"')

version-check:
ifneq ($(filter-out $(addon_version), $(el_version) $(el_pkg_version)),)
		$(error "Versions don't match. manifest.json and spookfox.el must have same version.")
else ifeq ($(addon_version),$(master_version))
